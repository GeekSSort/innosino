...
import type { ContactPageData, SiteSettings } from "@/sanity/queries";

/*
 * ScrollTrigger has to be registered before it is passed to a tween. Without
 * it GSAP logs "Invalid property scrollTrigger" and drops the option, so the
 * hubs and standfirst animated on page load instead of on scroll — visible as
 * six console warnings and an entrance nobody ever saw.
 */
...
/**
 * Declared outside the page so it is one component type rather than a new one
 * on every render — a component created during render remounts, losing any
 * state it holds and defeating reconciliation.
 */
const CopyIcon = () => (
  ...
  useEffect(() => {
    const updateClocks = () => {
      ...
      try {
        ...
        /* Intl throws on a time zone the runtime has no data for. The clock
           keeps its previous value rather than the page failing around it. */
      }
    };
          ...
          <div className="ct-card" ref={cardRef}>
            ...
            <div className="ct-card__col">
              ...
              >
                ...
                >
                  <h2 className="ct-card__title">{page.form.title}</h2>
                  <p className="ct-card__lede">{page.form.lede}</p>
                </div>

                <div className="ct-points">
                  {page.form.points.map((item) => (
      ...
      <section className="flow-section" style={{ backgroundColor: "#F6F6F6" }}>
        <div className="container">
          <h2 className="section-heading" style={{ color: "#000000" }}>
            {page.faqHeading.lead}
            <span style={{ color: "#FF7018" }}>{page.faqHeading.accent}</span>
          </h2>
